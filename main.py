...
REWARD_IMAGE_URL = os.getenv("REWARD_IMAGE_URL", "https://i.postimg.cc/XNzZGCZY/5475f4b9-b4f6-4fc1-b072-a9be4132adb4.jpg")
...
def share_panel(campaign_id: int) -> InlineKeyboardMarkup:
    if BOT_USERNAME:
        ...
        return InlineKeyboardMarkup([[InlineKeyboardButton("📤 Je partage", url=url)]])
...
async def init_db():
    ...
    async with DB.acquire() as con:
        ...
        await con.execute("""
            ...
            WHERE chat_id=$1 AND (reward_image_url='' OR reward_image_url LIKE 'https://picsum.photos/%')
...
async def send_personal_share(user_id: int, context: ContextTypes.DEFAULT_TYPE, campaign_id: int):
    ...
    active_challenge = await DB.fetchrow("""
        ...
        WHERE rl.owner_id=$1 AND rl.delivered=FALSE AND rc.active=TRUE
    ...
    if active_challenge and int(active_challenge["campaign_id"]) != int(campaign_id):
        ...
        kb = InlineKeyboardMarkup([[InlineKeyboardButton("📤 Envoyer mon lien en cours", url=share_url)]])
        await context.bot.send_message(
            user_id,
            f"Tu as déjà un challenge en cours. Termine-le avant d’en ouvrir un autre.\n\n{invite}\n\nProgression : {count}/{required}",
            reply_markup=kb,
        )
        return
...
async def buttons(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    elif q.data in ("word_add", "word_del", "broadcast_group", "broadcast_private", "set_gofile", "set_reward_text", "set_reward_image"):
