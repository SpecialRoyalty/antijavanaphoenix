...
def user_share_panel() -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup([[InlineKeyboardButton("🔗 Créer mon lien personnalisé", callback_data="share")]])
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if update.effective_chat.type != "private":
        # On évite d'afficher le panel admin dans le groupe.
    ...
    if await is_admin(update, context):
        ...
        await update.message.reply_text(
            "Bienvenue. Clique pour recevoir ton lien personnalisé de partage.",
            reply_markup=user_share_panel(),
        )


async def admin(update: Update, context: ContextTypes.DEFAULT_TYPE):
    # Commande de secours. Le vrai accès admin se fait maintenant avec /start en privé.
    if update.effective_chat.type != "private":
        await delete_safely(context, update.effective_chat.id, update.effective_message.message_id)
        return
                ...
                if inserted.endswith("1"):
                    ...
                    if count >= REWARD_REQUIRED_JOINS:
                        ...
                        if not delivered:
                            ...
                            if gofile:
                                try:
                                    await context.bot.send_message(owner, f"🎁 Bravo ! Voici ton lien privé :\n{gofile}")
                                    await DB.execute("UPDATE reward_links SET delivered=TRUE WHERE chat_id=$1 AND owner_id=$2", GROUP_ID, owner)
                                except Exception:
                                    pass


async def send_personal_share(user_id: int, context: ContextTypes.DEFAULT_TYPE):
    row = await DB.fetchrow("SELECT invite_link, joins_count FROM reward_links WHERE chat_id=$1 AND owner_id=$2", GROUP_ID, user_id)
    if not row:
        link = await context.bot.create_chat_invite_link(GROUP_ID, name=f"reward_{user_id}_{int(time.time())}")
        await DB.execute("INSERT INTO reward_links(chat_id,owner_id,invite_link) VALUES($1,$2,$3)", GROUP_ID, user_id, link.invite_link)
        invite, count = link.invite_link, 0
    else:
        invite, count = row["invite_link"], row["joins_count"]

    share_text = f"Rejoins ce groupe, il y a une récompense à débloquer : {invite}"
    ...
    await context.bot.send_message(
        user_id,
        f"Partage ce lien personnalisé. Quand {REWARD_REQUIRED_JOINS} personnes rejoignent avec TON lien, tu reçois le lien privé.\n\n{invite}\n\nProgression : {count}/{REWARD_REQUIRED_JOINS}",
        reply_markup=kb,
    )
...
async def buttons(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if q.data == "share":
        try:
            await send_personal_share(q.from_user.id, context)
            await q.answer("Lien personnalisé envoyé en privé.", show_alert=True)
        except Exception:
            start_link = f"https://t.me/{BOT_USERNAME}?start=share" if BOT_USERNAME else "le bot en privé"
            await q.answer(f"Ouvre d’abord le bot en privé avec /start : {start_link}", show_alert=True)
        return
    ...
    if q.data in ("toggle_on", "toggle_off"):
    ...
    elif q.data in ("word_add", "word_del", "broadcast_group", "broadcast_private", "set_gofile"):
        ...
        caption = f"🎁 Récompense disponible !\n\nClique sur “Je partage” pour recevoir ton lien personnalisé. Dès que {REWARD_REQUIRED_JOINS} personnes rejoignent avec ton lien, tu reçois le lien privé."
